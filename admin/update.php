<?php
    ...
    if(!empty($_POST)) 
    {
        ...
        { 
            ...
            if($isImageUpdated)
            {
                $statement = $db->prepare("UPDATE products  set p_name = ?, p_description = ?, p_price = ?, p_category = ?, p_image = ? WHERE p_id = ?");
                $statement->execute(array($name,$description,$price,$category,$image,$id));
            }
            else
            {
                $statement = $db->prepare("UPDATE products  set p_name = ?, p_description = ?, p_price = ?, p_category = ? WHERE p_id = ?");
        ...
        {
            $db = Database::connect();
            $statement = $db->prepare("SELECT * FROM products where p_id = ?");
